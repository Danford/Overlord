...
// verify that it's a valid key & id
    ...
    $q = "SELECT COUNT(*) FROM `confirmation_key` WHERE `user_id`='".$id."' and `confirmation_key`='".$key."' and `type`='0'" ;
...
// at this point, we have an valid id/key combo    
    ...
    $db->update( "DELETE FROM `confirmation_key` WHERE `user_id`='".$id."' and `confirmation_key`='".$key."' and `type`='0'" ) ;
